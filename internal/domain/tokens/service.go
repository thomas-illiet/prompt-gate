...
// StartCleanup starts a background goroutine that periodically marks expired tokens.
...
// markExpired marks expired persisted tokens as expired.
func (s *Service) markExpired(ctx context.Context) {
	now := time.Now().UTC()
